...
exports.getProfilesHomepageTweets = async (req, res, next) => {
  try {
    const { homepageTweets } = await Profile.findOne({
      user: req.user.id,
    }).select('homepageTweets');
    const tweets = await Tweet.find({
...
exports.createTweet = async (req, res, next) => {
  ...
  try {
    ...
    const { followers } = await Profile.findOne({ user: req.user.id }).select(
      'followers'
    );
  ...
  try {
    ...
    if (req.user.id !== tweet.user.toString()) {
      return res.status(401).json({
        errors: [{ msg: 'You are not allowed to delete that tweet' }],
      });
    }
    ...
    await Comment.remove({ tweet: tweet_id });
    await Profile.updateOne(
      { user: req.user.id },
